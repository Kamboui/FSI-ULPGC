...
with tf.Session() as sess:
    ...
    while True:
        ...
        result = sess.run(y, feed_dict={x: frame})
        if np.argmax([1. , 0., 0.]) == np.argmax(result):
            print("-----|| Esto es una piedra ||-------")

        if np.argmax([0., 1., 0.]) == np.argmax(result):
            print("-----|| Esto es un teclado ||-------")

        if np.argmax([0., 0., 1.]) == np.argmax(result):
            print("-----|| Esto es una regleta ||-------")
